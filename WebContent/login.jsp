...
<link type="text/css" rel="stylesheet"
	href="unitime/gwt/standard/standard.css">
...
<BODY class="bodyMain" onload="document.forms[0].j_username.focus();">
	<tt:form-factor value="unknown">
		<span id='UniTimeGWT:DetectFormFactor' style="display: none;"></span>
	</tt:form-factor>
 ...
 %>
	...
	<FORM name="f" action="<c:url value='j_spring_security_check'/>"
		...
		<INPUT type="hidden" name="cs" value="login"> <INPUT
			type="hidden" name="menu"
			value="<%=request.getParameter("menu") == null ? "" : request.getParameter("menu") %>">
		<INPUT type="hidden" name="target"
			value="<%=request.getParameter("target") == null ? "" : request.getParameter("target") %>">
					...
					<div class="BackGround">
						...
						<div class="txtField">
							<input type='text' name='j_username'
								value='<c:if test="${not empty SPRING_SECURITY_LAST_USERNAME}"><c:out value="${SPRING_SECURITY_LAST_USERNAME}"/></c:if>'
						...
						<div class="H20px"></div>
						<div class="bottom">
							<input id="submit" name="submit" type="image"
								src="images/login_bg_3.jpg" border="0" align="right"
								value="log in" alt="Submit login information.">
						</div>
					</div>
				</div>
		</span> <c:if test="${SUGGEST_PASSWORD_RESET}">
				<span class='forgot'><a href='gwt.jsp?page=password&reset=1'
					class='unitime-FooterLink'>Forgot your password?</a></span>
			</c:if>
		</span>
	</FORM>
	...
	<jsp:include flush="true"
		page='<%=ApplicationProperties.getProperty("tmtbl.footer.external")%>' />
	<% } %>
